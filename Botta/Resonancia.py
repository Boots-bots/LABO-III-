...
#Transferencia 
 #C 1uF /F 0.010 H /R 1200 ohm  
 # w0 = 10000, f = 1591.5, dw = 120000, Q = 0.08)

ruta = "C:/Users/faust/Documents/UBA/Actividades/Laboratorio/3/Datos/Resonancia/"

dfT = pd.read_csv(ruta + "RLC_Transferencia(1uF).csv", index_col=["Frecuencias"])

frecuencias = np.unique(dfT.index)
...
for frec in frecuencias:
    tiempo = dfT.loc[frec]["Tiempo"].values
    vch1 = dfT.loc[frec]["VoltajeCH1"].values
...
plt.plot(frecuencias, corr ,'.b', label="Mediciones") #prop
plt.ylabel("I [mA]")
...
plt.axvline(x= 10000/(2*np.pi), color='g', linestyle='--', label = "w0") 
plt.axvline(x= 130000/(2*np.pi), color='g', linestyle='--', label = "dw") 
plt.plot(frecuencias, pot, '.b', label="Mediciones") #prop
...
plt.axvline(x= 10000/(2*np.pi), color='g', linestyle='--', label = "w0") 
plt.axvline(x= 130000/(2*np.pi), color='g', linestyle='--', label = "dw") 
plt.errorbar(frecuencias, dif_amp, dif_amp_std, fmt='.', color="b", label="Mediciones")
